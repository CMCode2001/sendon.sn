...
const coords = [
  {
    icon: Phone,
    label: "TÉLÉPHONE",
    value: "+221 78 796 44 36",
    href: "tel:+221787964436",
  },
  {
    icon: Mail,
    label: "EMAIL",
    value: "support@sendon.sn",
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setStatus("loading")
    // Simulate send — swap with real API/Supabase call as needed
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Header dark */}
      <div className="relative overflow-hidden bg-[#0f0f0f] pb-32 pt-36">
        ...
        <Container className="relative">
          ...
          >
            <h1 className="font-display text-[42px] font-extrabold leading-tight tracking-tight text-white sm:text-[56px]">
              Contactez-nous
            </h1>
        ...
        <Container>
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_380px]">
            ...
            <motion.div
              ...
              className="rounded-[24px] bg-white p-8 shadow-[0px_8px_40px_rgba(0,0,0,0.10)] md:p-10"
            >
              {status === "success" ? (
                ...
                <>
                  <h2 className="font-display text-[22px] font-bold text-[#0c0a09]">
                  ...
                  <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-5">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div className="flex flex-col gap-1.5">
                        <label className="text-[13px] font-semibold text-[#0c0a09]">
                          Nom complet
                        </label>
                        <input
                          required
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        ...
                        <svg
                          className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#6b5f58]"
                          width="16"
                          height="16"
                          viewBox="0 0 16 16"
                          fill="none"
            ...
            <motion.div
              ...
              className="rounded-[24px] bg-white p-8 shadow-[0px_8px_40px_rgba(0,0,0,0.10)]"
            >
              <h2 className="font-display text-[22px] font-bold text-[#0c0a09]">
                Coordonnées
              </h2>
              ...
              <ul className="mt-6 flex flex-col gap-6">
                {coords.map(({ icon: Icon, label, value, href }) => (
                  <li key={label} className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[11px] font-bold uppercase tracking-widest text-[#9d9187]">
                        {label}
                      </p>
                      {href ? (
                        <a
                          href={href}
                          className="font-display text-lg font-bold text-[#0c0a09] transition-colors hover:text-rouge-500"
                        >
                          {value}
                        </a>
                      ) : (
                        <p className="font-display text-lg font-bold text-[#0c0a09]">
                          {value}
                        </p>
                      )}
                    </div>
              ...
              {/* Map placeholder */}
              <div className="mt-8 overflow-hidden rounded-[16px] bg-[#f2f0ed]">
                <iframe
                  title="SenDon Dakar"
          ...
          <div className="mt-8 pb-4">
            <Link
              to="/"
              className="inline-flex items-center gap-2 text-sm font-semibold text-[#6b5f58] transition-colors hover:text-rouge-500"
            >
              ← Retour à l'accueil
            </Link>
          </div>
